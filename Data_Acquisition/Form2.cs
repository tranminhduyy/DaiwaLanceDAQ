...
namespace Data_Acquisition1
{
    public partial class Form2 : Form
    {
        ...
        SqlConnection cnn = new SqlConnection(@"Data Source=TTBTECH;Initial Catalog=DAQ;Integrated Security=True");
        //SqlConnection cnn = new SqlConnection(@"Data Source=LAPTOP-KTU4214O;Initial Catalog=DAQ;Integrated Security=True");
        string sql;
        ...
        private void ketnoicsdl()
        {
            ...
            cnn.Open();
            if (thoigian.Day == 1)
                sql = "use DAQ delete from DAILY declare @dateb datetime set @dateb = getdate() declare @start date = DATEADD(MONTH, DATEDIFF(MONTH, @dateb-1, GETDATE())-1, @dateb-1) declare @end date = DATEADD(MONTH, DATEDIFF(MONTH, @dateb-1, GETDATE())-1, @dateb-1) insert into DAILY select * from VARIABLE_DAQ where DATE_TIME between @start and @end select * from DAILY";
            else sql = "use DAQ delete from DAILY declare @dateb datetime set @dateb = getdate() declare @start date = DATEADD(MONTH, DATEDIFF(MONTH, @dateb-1, GETDATE())-0, @dateb-1) declare @end date = DATEADD(MONTH, DATEDIFF(MONTH, @dateb-1, GETDATE())-0, @dateb-1) insert into DAILY select * from VARIABLE_DAQ where DATE_TIME between @start and @end select * from DAILY";
        ...
        {
            //sql = "select * from VARIABLE_DAQ";
           // DateTime TimeStamp = DateTime.Now;
           // label_minute.Text = TimeStamp.Minute.ToString();
        }
